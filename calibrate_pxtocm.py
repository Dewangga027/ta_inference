...
# Kalibrasi manual berdasarkan kendaraan referensi di ROI
# Contoh: City Car panjang 330cm terlihat dengan width 120px → 330/120 = 2.75
PIXEL_TO_CM = 2.75
...
print("Loaded columns:", vehicle_df.columns.tolist())
print(f"[INFO] YOLO model labels: {YOLO(MODEL_PATH).names}")  # Debugging
...
def classify_and_weight(label, length_cm):
    ...
    if not rows.empty:
        rows = rows.copy()
        rows["diff"] = (rows["length_cm"] - length_cm).abs()
        best_match = rows.loc[rows["diff"].idxmin()]
        return best_match["vehicle_category"], int(best_match["weight_kg"])
    return None, 0
...
while cap.isOpened():
    ...
    for box in results.boxes:
        ...
        print(f"[CALIB] {label} | w_px={x2 - x1}, h_px={y2 - y1}")
